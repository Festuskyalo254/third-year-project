    ...
    if selected_item in symptom_list:
        ...
        text_area.insert(tk.END, f"Recommendation: {selected_symptom['recommendation']}\n")
            ...
            text_area.insert(tk.INSERT, "Selected symptom not found in the list\n"
                                        "find solution  from the  database.")
...
connection.connect()
c.execute("SELECT DISTINCT symptom FROM diagnose")
symptoms_from_db = [symptom[0] for symptom in c.fetchall()]
connection.commit()

# Symptom lists
symptom_lists2 = [
    {"symptoms": symptoms_from_db}
]


# Entry box for the symptom option
symptom_combobox_2 = ttk.Combobox(window, values=symptoms_from_db, width=40, height=2)
symptom_combobox_2.grid(row=5, column=1, columnspan=2, pady=10, padx=10)
...
def get_solution_from_database():
    ...
    user_entered_symptom = symptom_combobox_2.get().strip()
